...
slug: makefiles
summary: A set of recipes for using Makefiles for web development.
date: 2022-12-31
---
...
I leverage this conventionality to create consistent commands between different projects. For example, in every new project, I add a `make dev` command, which [auto-downloads dependencies](#auto-installing-node_modules), starts a dev server, and watches for changes, no matter the language.
...
- `make help` lists all the commands in a Makefile ([discussed here](#self-documenting-makefiles))

This "one command" and "one interface" is highly beneficial for teammates too.
They can get started quickly, and if they want to learn more or tinker, the commands and dependency relationships are all documented in a single place.

### Recipes

Other articles cover the basics of how to write a Makefile in great depth,[^2] so I'll leave syntax aside and instead focus on sharing some common techniques that I use:

- [Tasks without dependencies](#tasks-without-dependencies)
- [Referencing node_modules](#referencing-node_modules-binaries)
- [Auto-installing node_modules](#auto-installing-node_modules)
- [Skipping re-runs of slow tasks](#skipping-re-runs-of-slow-tasks)
- [Using environment variables](#using-environment-variables)
- [Configuring tasks with default variables](#configuring-tasks-with-default-variables)
- [Self-documenting Makefiles](#self-documenting-makefiles)
- [Parallel dev servers](#parallel-dev-servers)
- [Hermetic environments](#hermetic-environments)
...
This explicitness has the benefit of throwing an error if the local `prettier` isn't installed (but it should be if you [set up node_modules as a dependency](#auto-installing-node_modules)).

There are techniques to modify your `$PATH` inside Makefiles, but they don't seem to work consistently with the version of `make` that ships with macOS.[^3] Better to keep things simple.
...
To use env variables you've defined in a local file, like a `.env`, you can
use the following block at the top of your Makefile:
...
- Shell background jobs with `<command> &` suffers the same process group issue, plus can sometimes leave processes hanging around after you Ctrl+C out.
- External binaries like GNU's `parallel` command, or [overmind](https://github.com/DarthSim/overmind) work, but they require separate installation steps. You can automate this, but it's not ideal.

Thankfully, `make` doesn't care: we can just pop into language-specific tools that are suitable for the job.

In Go projects, I'll use a custom Go script. In Node projects, I'll just install the [concurrently](https://www.npmjs.com/package/concurrently) npm package:

```
yarn add --dev concurrently
```

Then use it from your Makefile. Passing the full `./node_modules/.bin` path before your npm-based tools, and use the `-k` flag to kill all processes when one fails:
...
_Update_ — I've since published [tandem](https://github.com/rosszurowski/tandem), which tries to solve this problem with an easy Makefile-specific installation step.

### Hermetic environments

Building on [the auto-installing node_modules technique](#auto-installing-node_modules): you can get fancy with dependencies to download local versions of your _entire_ toolchain. This way everyone runs the same version of everything, without the slowness of using Docker.
...
[^1]: Here's a [more complete Makefile example for Kirby](https://gist.github.com/rosszurowski/24bdc2cce3bd440e6210c9c7d7164745) if you're interested in seeing how this approach expands.
[^2]: I've found [this article](https://earthly.dev/blog/make-tutorial/) and [this one](https://www.olioapps.com/blog/the-lost-art-of-the-makefile/) to be decent introductions to the Makefile syntax.
[^3]: More details about the specific weirdness are [in this gist](https://gist.github.com/rosszurowski/eee05cebb67fd869b7db1914b844db9c).
